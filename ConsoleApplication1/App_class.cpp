...
void App_class::Signal_elevator_enter(string& s) {}

void App_class::Signal_elevator_exit(string& s) {}

void App_class::Signal_elevator_move(string& s) {}

void App_class::Signal_passenger_coming(string& s) {}

void App_class::Signal_to_control_panel(string& s) {}
...
void App_class::bild_tree_objects() {
	...
	Control_panel* control_panel = new Control_panel(this, "object_remote_control");

	Input_class* input = new Input_class(this, "object_read_command");
	...
	this->set_connect(SIGNAL_D(App_class::Signal_for_condition_elevator), elevator,
		HANDLER_D(Elevator::Handler_condition));
	this->set_connect(SIGNAL_D(App_class::Signal_to_control_panel), control_panel,
		HANDLER_D(Control_panel::Handler_from_system));
	input->set_connect(SIGNAL_D(Input_class::Signal_input), this,
		HANDLER_D(App_class::Handler_exec_sistem));
	control_panel->set_connect(SIGNAL_D(Control_panel::Signal_elevator), elevator,
		HANDLER_D(Elevator::Handler_from_control_panel));
	for (int i = 1; find_s_object(to_string(i)) != nullptr; i++) {
		Base_class* floor = find_s_object(to_string(i));
		floor->set_connect(SIGNAL_D(Floor::Signal_enter_passenger),
			elevator, HANDLER_D(Elevator::Handler_enter_passenger));
		floor->set_connect(SIGNAL_D(Floor::Signal_condition), find_s_object("object_display_screen"),
			HANDLER_D(Output_class::Handler_output));
		this->set_connect(SIGNAL_D(App_class::Signal_for_condition_floor), floor,
...
void App_class::Handler_construct_sistem(string& s_command) {
	...
	((Elevator*)(get_pointer_object("/object_remote_control/object_elevator")))->set_capacity(stoi(s_command));
	...
	void App_class::change_name_objects() {
		//меняем имена и объектов этажей и пассажирах на этих этажах
	...
	void App_class::Handler_exec_sistem(string & s_command) {
		...
		Elevator* elevator = (Elevator*)get_pointer_object("/object_remote_control/object_elevator");
		...
		if (s_command != "") {
			...
			if (message == "E")
				emit_signal(SIGNAL_D(App_class::Signal_for_condition_elevator), message);

			if (message == "C") {
				s_command.erase(s_command.begin(), s_command.begin() + 23);
				emit_signal(SIGNAL_D(App_class::Signal_for_condition_floor), s_command);
			}
			...
			if (message == "P") {//команда пассажира
				...
				if (s_command[0] == 'c') {//команда показа состояния пассажира
					...
					if (passenger == nullptr)//если пассажир не найден, система об этом говорит, ...
						...
						set_connect(SIGNAL_D(App_class::Signal_for_condition_passenger), passenger,
							HANDLER_D(Passenger::Handler_condition));
						emit_signal(SIGNAL_D(App_class::Signal_for_condition_passenger), s_command);
						delete_connect(SIGNAL_D(App_class::Signal_for_condition_passenger), passenger,
							HANDLER_D(Passenger::Handler_condition));
					}
					...
					passenger->set_connect(SIGNAL_D(Passenger::Signal_condition), find_s_object("object_display_screen"),
						HANDLER_D(Output_class::Handler_output));
					passenger->set_connect(SIGNAL_D(Passenger::Signal_coming_to_the_floor), floor,
						HANDLER_D(Floor::Handler_passengers));
					passenger->set_connect(SIGNAL_D(Passenger::Signal_control_panel), find_s_object("object_remote_control"),
						HANDLER_D(Control_panel::Handler_from_passenger));
					//возбуждение сигнала подхода пассажира к этажу
					set_connect(SIGNAL_D(App_class::Signal_passenger_coming), passenger,
						HANDLER_D(Passenger::Handler_coming_to_the_floor));
					emit_signal(SIGNAL_D(App_class::Signal_passenger_coming), str_number_floor);
		...
		//обработка такта
		{
			emit_signal(SIGNAL_D(App_class::Signal_elevator_exit), message);//старые пассажиры выходят
			emit_signal(SIGNAL_D(App_class::Signal_elevator_enter), message);//новые пассажиры заходят
			emit_signal(SIGNAL_D(App_class::Signal_to_control_panel), message);//лифту подаётся новый пункт назначения
	...
	void App_class::Signal_sistem(string & s_command) {//сигнал окончания
		...
		if (s_command == "Turn off the system" || s_command == "SHOWTREE") s_command = "\nTurn off the system";
	...
	void App_class::Signal_condition(string & message) {//сигнал состояния системы
		...
		message = "\n" + to_string(time) + ": Elevator: "
			+ to_string(((Elevator*)(get_pointer_object("/object_remote_control/object_elevator")))
				->get_level()) + " " + to_string(((Elevator*)(get_pointer_object("/object_remote_control/object_elevator")))
					->get_people()) + "  Floors:";
		...
		for (int i = 1; find_s_object(to_string(i)) != nullptr; i++) {
			...
			if (count_up_people + count_down_people != 0)
				message += " (" + floor->get_object_name() + ": " + to_string(count_up_people) + "-"
				+ to_string(count_down_people) + ")";

		}
	}

	void App_class::Signal_from_floors(string & s_command) {//сигнал от этажей с кнопками
		Elevator* elevator = (Elevator*)get_pointer_object("/object_remote_control/object_elevator");
		int now_level = elevator->get_level();//нынешний этаж лифта
		/*
			Если лифт движется вверх, то сначала проверяются все этажи выше текущего. Если находится этаж с включённой
			кнопкой "ВВЕРХ", то лифт передаётся новый пункт назначения. Если такого этажа не находится, то проверяются
			этажи выше текущего на наличие кнопки "ВНИЗ". Если такого этажа не находится, то проверяются этажи ниже
			текущего на наличие кнопки "ВНИЗ".

			Если лифт движется вниз, то сначала проверяются все этажи нижа текущего. Если находится этаж с включённой
			кнопкой "ВНИЗ", то лифт передаётся новый пункт назначения. Если такого этажа не находится, то проверяются
			этажи ниже текущего на наличие кнопки "ВВЕРХ". Если такого этажа не находится, то проверяются этажи ниже
			текущего на наличие кнопки "ВВЕРХ".

			Если лифт стоит, то проверяются все этажи на наличие любых сигналов.

			Ситуация, когда лифт едет сначала вниз, а потом вверх или наоборот, называется change_direction и обозначается
			символом [c].

			Ситуация, когда лифт едет вверх, доставляет пассажиров, а затем продолжает ехать вверх, чтобы забрать и
			доставить пассажира вниз или наоборот, называется reverse_direction и обозначается символом [r]. Так же сюда
			подходят ситуации, когда лифт из положения стоп едет выше своего нынешнего этажа, чтобы забрать и
			доставить пассажира	вниз или наоборот.

			Ситуация, когда лифт стоит и получает сигнал, называется stop_direction и обозначается символом [s].

			Остальные ситуации, когда лифт "подхватывает" по пути пассажиров называются usual_direction и обозначаются
			символом [u].

			Ситуация, когда сигналов от этажей нет, называется NON_direction и обозначается символом [N].
		*/
		...
		if (elevator->get_direction() == Elevator::stop) {
			for (int i = 1; i <= floors; i++) {
				...
				if (floor->get_up_button() || floor->get_down_button()) {
					s_command = "s";
					if ((floor->get_up_button() && elevator->get_level() > i)
						|| (floor->get_down_button() && elevator->get_level() < i))
						s_command = "r";
...
void App_class::change_name_objects() {
	//меняем имена и объектов этажей и пассажирах на этих этажах
	for (int i = 1; i <= floors; i++)
		find_s_object(to_string(i))->rename_object("object_floor_" + get_object_name());
}
