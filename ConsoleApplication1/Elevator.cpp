...
void Elevator::Signal_condition(string& message) {
	message = "\nElevator condition: " + to_string(level_now) + " " + to_string(people) + " ";
...
void Elevator::Handler_passengers(string& s_command) {
	...
	if ((switcher == 'u' && direction == Elevator::up && level_max < number_floor) ||
		(switcher == 'u' && direction == Elevator::down && level_max > number_floor))
...
void Elevator::exit_passenger(string& s) {
	...
	if ((people == 0 && ppl != people) || (reverse_direction && level_now == level_max)) {
		reverse_direction = false;
		emit_signal(SIGNAL_D(Elevator::Signal_to_system), command);
	}
}

void Elevator::Signal_to_system(string& s) {}

void Elevator::Signal_to_passenger_about_control_panel(string& s) {}
...
void Elevator::Handler_enter_passenger(string fio) {
	...
	while (fio != "") {
		if (fio[0] != '/')
			...
			set_connect(SIGNAL_D(Elevator::Signal_to_passenger_about_control_panel), passenger,
				HANDLER_D(Passenger::Handler_control_panel));
			emit_signal(SIGNAL_D(Elevator::Signal_to_passenger_about_control_panel), fio_passenger);
			delete_connect(SIGNAL_D(Elevator::Signal_to_passenger_about_control_panel), passenger,
				HANDLER_D(Passenger::Handler_control_panel));
			people++;
			fio_passenger = "";
		}
...
void Elevator::enter_passenger(string& s) {
	...
	set_connect(SIGNAL_D(Elevator::Signal_to_floor), get_pointer_object("/" + to_string(level_now)),
		HANDLER_D(Floor::Handler_enter_passenger));
	emit_signal(SIGNAL_D(Elevator::Signal_to_floor), command);
	delete_connect(SIGNAL_D(Elevator::Signal_to_floor), get_pointer_object("/" + to_string(level_now)),
		HANDLER_D(Floor::Handler_enter_passenger));
}
...
void Elevator::Handler_from_control_panel(string destination_floor) {
	...
	if ((direction == Elevator::up && d_f > level_max)
		|| (direction == Elevator::down && d_f < level_max) || direction == Elevator::stop)
		level_max = d_f;
