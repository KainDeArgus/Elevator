...
void Floor::Signal_condition(string& message) {
	...
	for (int j = 0; j < down_queue.size(); j++)
		message += " " + down_queue[j];
}
...
void Floor::Signal_enter_passenger(string& direction_elevator) {
	...
	while (direction_elevator[0] != ' ') {
		str_people += direction_elevator[0]; direction_elevator.erase(direction_elevator.begin());
	}
...
void Floor::Handler_passengers(string& dir) {
	...
	if (direcrion == 'u') {
		...
		if (!down_button)down_button = true;
